...
class AsynchronousChannelBase
{
	public
	{
		...
		{
			if(register(AIOEventType.OP_READED,  cast(void*)handle, attachment, buffer)==false)
			{
		...
		{
			if(register(AIOEventType.OP_WRITEED,  cast(void*)handle, attachment, buffer)==false)
			{
				handle.failed(attachment);
			}
		}
	...
	protected bool register(int ops, void* handle, void* attchment, ByteBuffer obj = null)
    {
        if (!checkVailid(ops))
		{
            return false;
		...
		return  _key.interestOps(ops, isNew);
    }
